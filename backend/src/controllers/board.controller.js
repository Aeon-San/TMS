...
export const createBoard = async (req, res) => {
  ...
  try {
    const board = new Board({
      ...
      description: sanitizeText(description, { maxLength: 500 }),
      owner: req.userId,
...
export const inviteUserToBoard = async (req, res) => {
  ...
  try {
    ...
    if (req.user.role !== 'admin' && !isOwner && memberRole !== 'admin') {
      return res.status(403).json({ message: 'Only board owners and admins can invite users' });
    }

    const userToInvite = await User.findOne({ email: String(email).trim().toLowerCase() });
    if (!userToInvite) {
    ...
    const invitation = new BoardInvitation({
      ...
      invitedUser: userToInvite._id,
      role: role || 'member',
      message: sanitizeText(message, { maxLength: 300 }),
    });
